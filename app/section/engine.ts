import type { ModePose, SectionCable, SectionInput, SectionResult, TypeReseau } from "./types";
...
const CAPACITES: Record<ModePose, Record<SectionCable, number>> = {
  ...
  A1: { 1.5: 13,  2.5: 17.5, 4: 23,  6: 29,  10: 39,  16: 52,  25: 68,  35: 84,  50: 99  },
  B1: { 1.5: 15,  2.5: 20,   4: 27,  6: 34,  10: 46,  16: 62,  25: 80,  35: 99,  50: 118 },
  B2: { 1.5: 14.5,2.5: 19.5, 4: 26,  6: 33,  10: 44,  16: 58,  25: 73,  35: 90,  50: 107 },
  C:  { 1.5: 17.5,2.5: 24,   4: 32,  6: 41,  10: 57,  16: 76,  25: 96,  35: 119, 50: 144 },
  E:  { 1.5: 19.5,2.5: 27,   4: 36,  6: 46,  10: 63,  16: 85,  25: 110, 35: 137, 50: 167 },
};
...
function calculerIntensite(puissanceW: number, tensionV: number, reseau: TypeReseau): number {
...
function calculerChuteTension(
  ...
  if (intensiteA <= 0 || tensionV <= 0 || longueurM <= 0 || sectionMm2 <= 0) {
    return { chuteTensionV: 0, chuteTensionPourcent: 0 };
  }

  const chuteTensionV =
    reseau === "monophase"
      ? (2 * RESISTIVITE_CUIVRE * longueurM * intensiteA) / sectionMm2
      : (Math.sqrt(3) * RESISTIVITE_CUIVRE * longueurM * intensiteA) / sectionMm2;
...
export function calculerSection(input: SectionInput): SectionResult {
  const { puissanceW, tensionV, reseau, longueurCableM, circuit, modePose } = input;
  ...
  let { chuteTensionV, chuteTensionPourcent } = calculerChuteTension(
    intensiteCalculeeA, tensionV, longueurCableM, sectionRecommandee, reseau
  );
